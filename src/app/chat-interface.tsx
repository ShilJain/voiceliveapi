...
const ChatInterface = () => {
  ...
  // Add state variables for customer details
  ...
  const handleConnect = async () => {
    if (!isConnected) {
      try {
        ...
        // Build enhanced instructions that include customer details
        let enhancedInstructions = instructions || "";
        
        // Add customer details to instructions if they are provided
        const customerDetails = [];
        if (customerName) customerDetails.push(`Customer Name: ${customerName}`);
        if (loanAmount) customerDetails.push(`Loan Amount: ${loanAmount}`);
        if (dueDate) customerDetails.push(`Due Date: ${dueDate}`);
...
${enhancedInstructions}`;
        ...
        const session = await clientRef.current.configure({
          instructions: enhancedInstructions?.length > 0 ? enhancedInstructions : undefined,
      ...
      >
        <div className="flex-1 overflow-y-auto">
          <Accordion type="single" collapsible className="space-y-4">
            {/* Customer Details - New section added above Instructions */}
            ...
            {/* Connection Settings - Existing section */}
